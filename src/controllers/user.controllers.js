...
const generateAccessAndRefreshToken = async (userId) => {
  try {
    ...
    throw new ApiError(
      500,
      "Some thing went wrong whie genratin refresh and access token"
    );
  }
};
...
const updateUserAvatar = asyncHandler(async (req, res) => {
  ...
  if (oldAvatarPublicId) {
    try {
      await uplodeOnCloudinary.uploader.destroy(oldAvatarPublicId);
    } catch (error) {
...
const updateUserCoverImage = asyncHandler(async (req, res) => {
  ...
  if (oldCoverPublicId) {
    try {
      await uplodeOnCloudinary.uploader.destroy(oldCoverPublicId);
    } catch (error) {
